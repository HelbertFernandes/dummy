 ...
 */
//$_SESSION=array();
function __autoload_old($class_name = false) {
	global $__autoload;
	if (!@$__autoload) {
		...
		if (@$_SERVER['SHELL']) {
			...
			$__autoload->path = substr($p['path'], -1) == '/' ? substr($p['path'], 0, strlen($p['path']) - 1) : dirname($p['path']);
...
function htmlScpChar($text, $quotes = ENT_NOQUOTES) {
	return htmlentities($text, $quotes, $GLOBALS['encode']);
}
function htmlConvert($str) {
	return mb_convert_encoding($str, $GLOBALS['encode']);
}
function pr($str) {
	print htmlConvert($str);
}
function prt($str) {
...
function json_encode_full($mixed, $t = '', $tab = '') {
	if (is_array($mixed)) {
		...
		return strlen($out) > 70 ? $out : str_replace(array($t, $lf), '', $out);
	}
	if (is_object($mixed)) {
...
class Parse_Conf {
	...
	function get_string($string = false) {
		global $__autoload;
		if (!$string) return false; { //Load Information
